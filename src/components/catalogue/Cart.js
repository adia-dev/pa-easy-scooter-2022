    ...
    if (!items || items.length == 0) {
        return (
            <div className="w-[375px] bg-white absolute right-0 z-30 shadow-xl rounded-2xl p-5 space-y-5">
                <h1 className="border-b">Panier</h1>
                <p className="text-xs text-gray-500">There is no item on your shopping cart right now</p>
            </div>
        )
    }
    return (
        <div className="w-[375px] bg-white absolute right-0 z-30 shadow-xl rounded-2xl p-5 overflow-y-scroll space-y-5">
            ...
            <div className="w-full mx-auto">
                <button className="bg-black text-white py-2 w-full rounded-lg">Passer à la commande</button>
            </div>
        </div>
    )
}
